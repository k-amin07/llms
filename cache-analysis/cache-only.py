...
llm_cache = SemanticCache(
    name="eval-zakir-cache-optimal",
    prefix="zakir-cache-optimal",
    redis_url=redis_url,
...
distance_thresholds = [0.7604798184]
res = {}
res_per_bin = {}
...
for key in grouped_data_keys:
    data = grouped_data[key][:1000]
    train_data, test_data = train_test_split(data, test_size=0.2, random_state=None)

    print("Processing train_data for group {}".format(key))
...
# Save to CSV
df.to_csv("res_per_bin.csv", index=False)
